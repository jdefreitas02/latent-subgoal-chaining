"""
train_hiql_lewm.py
HIQL + LeWM Hybrid: Hierarchical Implicit Q-Learning with World Model imagination.

Faithful port of ogbench HIQLAgent to the frozen-LeWM latent space:
  - Native 192D latents from a frozen JEPA encoder (SIGReg-regularised)
  - 10D learned goal representation φ(z_t, z_goal) → 10D, length-normalised
    (matches HIQL's state-dependent subgoal representation phi([s; g]))
  - HER goal sampling for HL: g_ultimate ~ uniform future state in episode,
    z_target = min(t + k, g_ult_idx)         (matches HGCDataset)
  - Dense reward r = -‖z_next − z_subgoal‖₂ kept for LL value updates,
    leveraging the SIGReg latent geometry — this is the LeWM advantage
    over pixel-based HIQL which uses sparse 0/-1 reward.
  - Phase 2: frozen LeWM rollouts generate synthetic transitions for LL
    training. Imagination anchors on real (z_t, z_subgoal) pairs so the
    192D dense reward remains well-defined.

Architecture:
  - GoalRep φ:    MLP on concat([z_t, z_goal]) → rep_dim, length-normalised.
  - High-Level (HL) GaussianActor: (z_t, g_ultimate 192D) → N(μ, σ) over rep_dim.
  - Low-Level  (LL) GaussianActor: (z_t, rep 10D)        → N(μ, σ) over 25D.
  - TwinValue V(z, rep): ensembled scalar.
  - Frozen LeWM predictor: k-step imagination rollouts in Phase 2.

Update order per step:
  A. Value (V + φ jointly) — bootstrapped IQL on (z_t, z_{t+k}) goals.
  A'. Auxiliary value update on HER (uniform-future) goals so V covers the
     goal distribution that HL queries it on.
  B. Low-Level Actor AWR (mixed real + synthetic in Phase 2).
  C. High-Level Actor AWR (real data only).
"""
...
class GoalRep(nn.Module):
    ...
    Output is length-normalised onto the sphere of radius √rep_dim, exactly
    matching ogbench HIQL's LengthNormalize on top of an MLP. Trained jointly
    with the value function via the value loss; a soft-updated target copy
    (`goal_rep_target`) is used in the V-target bootstrap, mirroring how
    HIQL's target_value carries its own goal_rep params.
    """
    ...
    def forward(self, z, z_goal):
        ...
        rep = self.net(x)
        # LengthNormalize: project onto sphere of radius √rep_dim.
        rep = rep / (rep.norm(dim=-1, keepdim=True) + 1e-8) * (self.rep_dim ** 0.5)
        return rep


class GaussianActor(nn.Module):
    """Goal-conditioned Gaussian actor with separable state/goal input dims.

    Used for both:
      - HL: state_dim=192, goal_dim=192,        output_dim=rep_dim, tanh_squash=False
      - LL: state_dim=192, goal_dim=rep_dim,    output_dim=25,      tanh_squash=True

    log_std clamped to [LOG_STD_MIN, LOG_STD_MAX] = [-5.0, 2.0].
    """
    ...
    def __init__(self, state_dim=192, goal_dim=192, output_dim=25,
        ...
        # Constant log-std matching ogbench const_std=True: one learnable scalar
        # per output dim. Prevents std→0 collapse in AWR.
    ...
    def sample(self, state, goal):
        """Reparameterized sample. Returns (action, log_prob, deterministic_mean)."""
...
class TwinValue(nn.Module):
    ...
    `rep` is the output of GoalRep φ(z, z_goal); the value MLP sees [z || rep].
    """
...
class RealOfflineCache:
    ...
    Sampling methods:
      sample_ll_batch(batch_size, k):
          (z_t, a, r, z_next, z_subgoal)
          z_subgoal = z_{t+k}  (clamped to episode end)
          r = -‖z_next − z_subgoal‖_2  (dense, no HER)

      sample_hl_batch(batch_size, k):
          (z_t, z_target, g_ultimate)         ← HER, matches HGCDataset
          g_ultimate ~ uniform future state in [t+1, n_wm]
          z_target   = min(t + k, g_ultimate_idx)

      sample_value_her_batch(batch_size):
          (z_t, z_next, g_her)                ← uniform-future HER goal
          For the auxiliary value update so V covers the HL query distribution.

    k-step lookups are fully vectorised via a flat WM-latent tensor with
    per-episode offsets.
    """
    ...
    def __init__(self, all_latents, all_actions, action_scaler,
        ...
        wm_adim    = frameskip * raw_adim   # 5 × 5 = 25
    ...
    def _kstep_latents(self, idx, k):
        """Return z_{t+k} for each flat transition index. Clamped at episode end."""
    ...
    def _her_future_idxs(self, idx):
        """Uniform-random future WM-boundary index in [t+1, n_wm], per HGCDataset.

        Matches ogbench:
            distances = U(0,1)
            high_traj_goal_idxs = round(min(t+1, n_wm) * d  +  n_wm * (1-d))
        Returns: (g_idx_t, ep_id, t, n_wm) — all [B] long tensors on device.
        """
        ep_id = self.ep_ids[idx]
        t     = self.t_within[idx]
        n_wm  = self.ep_n_wm[ep_id]
    ...
    def sample_ll_batch(self, batch_size, subgoal_steps):
        """(z_t, a, r, z_next, z_subgoal) for LL value + AWR. Dense LeWM reward."""
    ...
    def sample_hl_batch(self, batch_size, subgoal_steps):
        """HER-style HL batch matching HGCDataset (actor_p_trajgoal=1.0,
        actor_geom_sample=False).

        Returns (z_t, z_target, g_ultimate) where:
          g_ultimate: uniform random future WM-boundary in [t+1, n_wm]
          z_target:   z at min(t + subgoal_steps, g_ultimate_idx)
        """
        idx = torch.randint(0, self.total, (batch_size,), device=self.device)
        z_t = self.z_t_flat[idx]
        ...
        g_ult     = self.wm_latents_flat[g_flat]
        z_target  = self.wm_latents_flat[tgt_flat]
        return z_t, z_target, g_ult

    def sample_value_her_batch(self, batch_size):
        """Auxiliary value batch with HER (uniform-future) goals.

        Returns (z_t, z_next, g_her). The goal is sampled to match the
        distribution that HL queries V on, so V coverage extends beyond the
        local k-step subgoals used by the LL value update.
        """
        idx    = torch.randint(0, self.total, (batch_size,), device=self.device)
        z_t    = self.z_t_flat[idx]
        z_next = self.z_next_flat[idx]
        ...
        g_flat = self.ep_wm_offsets[ep_id] + g_idx
        g_her  = self.wm_latents_flat[g_flat]
        return z_t, z_next, g_her


class SyntheticWMBuffer:
    """FIFO circular buffer for imagination-engine transitions.

    Stores (z_t, a, r, z_next, z_subgoal) where z_subgoal is the 192D anchor
    that conditioned the imagination rollout (rep is recomputed on the fly
    from φ(z, z_subgoal) during LL updates).
    """
    ...
    def push(self, z_t, a, r, z_next, z_subgoal):
        ...
        if end <= self.capacity:
            ...
            self.a[sl]         = a
            self.r[sl]         = r
            self.z_next[sl]    = z_next
            self.z_subgoal[sl] = z_subgoal
        else:
            overflow = end - self.capacity
            valid    = B - overflow
            self.z_t[self.ptr:]       = z_t[:valid];    self.z_t[:overflow]       = z_t[valid:]
            self.a[self.ptr:]         = a[:valid];      self.a[:overflow]         = a[valid:]
            self.r[self.ptr:]         = r[:valid];      self.r[:overflow]         = r[valid:]
            self.z_next[self.ptr:]    = z_next[:valid]; self.z_next[:overflow]    = z_next[valid:]
            self.z_subgoal[self.ptr:] = z_subgoal[:valid]; self.z_subgoal[:overflow] = z_subgoal[valid:]
    ...
    def sample(self, batch_size):
        ...
        return (self.z_t[idx], self.a[idx], self.r[idx],
                self.z_next[idx], self.z_subgoal[idx])


# =============================================================================
# 3. Imagination Engine
# =============================================================================

@torch.no_grad()
def run_imagination(wm_model, ll_actor, goal_rep, real_cache,
                    syn_buffer, k, batch_size, action_scale, device):
    """k-step WM rollouts → SyntheticWMBuffer.

    The rollout anchors on a real (z_t, z_subgoal=z_{t+k}) pair so the dense
    192D reward r = -‖z_next − z_subgoal‖₂ remains well-defined. LL is
    conditioned on rep = φ(z_curr, z_subgoal) recomputed each step (since the
    rep is state-dependent — matches HIQL's value forward call signature).

    HL is NOT used in imagination because its 10D output is not invertible to
    a 192D anchor needed for the dense reward. This means imagination explores
    LL behaviour around real-data subgoals; HL is updated on real data only.
    """
    idx        = torch.randint(0, real_cache.total, (batch_size,), device=device)
    z_curr     = real_cache.z_t_flat[idx].clone()
    z_subgoal  = real_cache._kstep_latents(idx, k)            # 192D anchor

    for _ in range(k):
        rep_sub = goal_rep(z_curr, z_subgoal)                 # state-dependent
        a, _, _ = ll_actor.sample(z_curr, rep_sub)            # [B, 25]
...
# =============================================================================
# 4. IQL Loss Functions
# =============================================================================

def _expectile(adv, diff, tau):
    """Expectile loss — port of ogbench HIQLAgent.expectile_loss.

    adv:  advantage from target network (sets the weight — double-DQN trick).
    diff: TD error from current network (q_i - V_i, receives gradients).
    """
    w = torch.where(adv >= 0,
...
def _value_step(value_net, value_target, goal_rep, goal_rep_target,
    ...
    """Joint V + φ value update (single optimizer step).

    Mirrors ogbench HIQLAgent.value_loss with the φ encoder applied:
      V(z, g)        := V_net(z, φ(z, g))
      V_target(z, g) := V_target_net(z, φ_target(z, g))
    ...
    rep_curr = goal_rep(z, z_goal)                            # gradients flow → φ
    v1, v2   = value_net(z, rep_curr)                         # gradients flow → V
...
def train_loop(
    ...
    save_dir          = './checkpoints_hiql_lewm',
    device            = 'cuda',
    log_interval      = 100,
    save_interval     = 1000,
):
    ...
    print(f"\n{'='*65}", flush=True)
    print(f"  HIQL+LeWM training (HER + {rep_dim}D goal_rep)", flush=True)
    ...
    print(f"  imagination_k  : {imagination_k}", flush=True)
    print(f"  syn_ratio      : {syn_ratio}", flush=True)
    print(f"  batch_size     : {batch_size}", flush=True)
    print(f"  rep_dim        : {rep_dim}", flush=True)
    ...
    with open(csv_path, 'w', newline='') as f:
        csv.writer(f).writerow([
            'step', 'phase',
            'value_loss', 'll_actor_loss', 'hl_actor_loss',
    ...
    for step in range(total_steps):
        ...
        # --------------------------------------------------------
        # Phase 2: run imagination to populate syn_buffer
        # --------------------------------------------------------
        if phase == 2:
            for _ in range(imagination_batches):
                run_imagination(
                    wm_model, ll_actor, goal_rep,
        ...
        # --------------------------------------------------------
        # Update A: IQL Value (V + φ jointly) on LL goals (z_{t+k})
        # --------------------------------------------------------
        ...
        # Auxiliary value update on HER (uniform-future) goals so V covers
        # the goal distribution HL queries.  Replaces the previous
        # episode-final goal coverage update — now matches the HER
        # distribution used by sample_hl_batch.
        ...
        # Soft-update value AND goal_rep targets together
        with torch.no_grad():
        ...
        # --------------------------------------------------------
        # Update B: Low-Level Actor AWR  (ogbench low_actor_loss)
        # adv = V(z_next, φ(z_next, z_sub)) - V(z, φ(z, z_sub))
        # LL is conditioned on rep_sub = φ(z, z_sub).detach()
        # (low_actor_rep_grad=False, matches state-based HIQL)
        # --------------------------------------------------------
        for _ in range(ll_grad_updates):
            if use_mixed:
                z, a, r, z_next, z_sub = _mixed_batch(
        ...
        # --------------------------------------------------------
        # Update C: High-Level Actor AWR  —  REAL DATA ONLY (HER goals)
        # adv = V(z_target, φ(z_target, g)) - V(z_t, φ(z_t, g))
        ...
        for _ in range(hl_grad_updates):
            ...
            with torch.no_grad():
                ...
                target_rep = goal_rep(z_t, z_tk)              # 10D, length-normed
        ...
        if step % log_interval == 0:
            ...
            print(
                ...
                f"V: {sum_v/d_ll:.4f} | "
                f"LL: {sum_ll/d_ll:.4f} | HL: {sum_hl/d_hl:.4f} | "
                f"SynBuf: {syn_buffer.size:,} | "
            ...
            with open(csv_path, 'a', newline='') as f:
                csv.writer(f).writerow([
                    step, phase,
                    sum_v/d_ll,
                    sum_ll/d_ll, sum_hl/d_hl,
                    syn_buffer.size, interval,
                ])
            sum_v = sum_ll = sum_hl = 0.0
            log_n = 0
            t_interval = now
...
def select_action(wm_model, hl_actor, ll_actor, goal_rep,
    ...
    """Encode → HL every k steps → LL every step.

    HL outputs a rep_dim-D mean (length-normalised onto sphere of radius
    √rep_dim, matching ogbench inference). LL receives this rep directly.

    Args:
        current_subgoal_rep: cached rep_dim-D subgoal tensor or None.
    Returns:
        action [25], new_subgoal_rep [rep_dim], z_curr [192]
    """
    with torch.no_grad():
        ...
        if current_subgoal_rep is None or step_counter % subgoal_steps == 0:
            _, _, new_rep = hl_actor.sample(
                z_curr.unsqueeze(0), g_ult.unsqueeze(0))      # deterministic mean
            new_rep = new_rep.squeeze(0)
            # Length-normalise onto sphere of radius √rep_dim, matching
            # ogbench HIQL.sample_actions:
            #   goal_reps = goal_reps / norm * sqrt(rep_dim)
            new_rep = new_rep / (new_rep.norm() + 1e-8) * (rep_dim ** 0.5)
        else:
            new_rep = current_subgoal_rep

        _, _, action = ll_actor.sample(
            z_curr.unsqueeze(0), new_rep.unsqueeze(0))        # deterministic mean
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='HIQL + LeWM Hybrid Training')
    ...
    parser.add_argument('--rep_dim',        type=int,   default=10,
                        help='Goal representation dimension (HIQL default: 10).')
    ...
    if not all_actions:
        raise RuntimeError(
            "Cache has no 'all_actions'. Rebuild the cache with the "
            "--save_actions flag in analyse_lewm_224.py.")
    ...
    syn_buffer = SyntheticWMBuffer(
        capacity=500_000, latent_dim=192, action_dim=25, device=device)
    ...
    # GoalRep φ(z, z_goal) → REP_DIM, length-normalised. Trained jointly
    # with the value function via the value loss.
    goal_rep = GoalRep(
    ...
    # LL: state z (192) + rep (REP_DIM) → action (25), tanh-squashed.
    ll_actor = GaussianActor(
        state_dim=LATENT_DIM, goal_dim=REP_DIM, output_dim=ACTION_DIM,
        hidden_dims=HIDDEN_DIMS,
        tanh_squash=True, action_scale=args.action_scale,
    ).to(device)

    # HL: state z (192) + raw goal g (192) → REP_DIM rep (no squash).
    ...
    value_net = TwinValue(
        latent_dim=LATENT_DIM, rep_dim=REP_DIM, hidden_dims=HIDDEN_DIMS,
    ).to(device)
    ...
    # V and φ trained together (φ is V's encoder, gradients must flow through both)
