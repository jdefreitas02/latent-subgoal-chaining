...
The main difficulty is \emph{distribution shift} between the two phases. Offline algorithms learn under a conservative prior --- behaviour-cloning regularisation, action-value clipping, or pessimism penalties --- that prevents the policy from straying too far from the data. When this prior is relaxed in the online phase, the critic's Q-values become unreliable on the new states the agent visits, and naive fine-tuning can destabilise or collapse the offline policy entirely. Cal-QL \cite{nakamoto2023calql} addresses this by explicitly calibrating Q-values during pretraining so the offline critic is well behaved as an online initialiser; AWAC \cite{nair2020awac} retains the offline behaviour-regularisation term throughout the online phase. In this thesis, Chapters \ref{chap:offline-online}--\ref{chap:mpc-distill} take a different route: rather than continue to interact with the environment, we use a learned world model in place of additional real interaction.
...
A learned world model is, in principle, two things at once: a generative environment in which the policy can collect imaginary experience, and an oracle the policy can query at decision time to look ahead. Chapter~\ref{chap:offline-online} concerns the first use; this thesis ultimately settles on the second, so we sketch the standard formulation here.
...
This combination --- a flow-matching actor producing $h$-step chunks and an chunked Q-function trained with the $h$-step Bellman target of Eq.~\eqref{eq:qchunking-target} --- is the agent we use throughout Chapters~\ref{chap:offline-online}--\ref{chap:mpc-distill}. The same paper also introduces a behaviour-regularised variant, QC-FQL, which is the closest published antecedent to our specific implementation choice and which we cover next.
...
The FQL recipe is what we use as the inner agent throughout Chapters~\ref{chap:offline-online}--\ref{chap:mpc-distill}, with $h=5$ chunked actions. Our extensions consist of (i) wrapping it in a WM environment (Chapter~\ref{chap:offline-online}) and (ii) modifying its actor-side training signal via MPC-distillation (Chapter~\ref{chap:mpc-distill}); the critic loss in Eq.~\eqref{eq:fql-critic} and the BC-flow loss in Eq.~\eqref{eq:bc-flow-bg} remain unchanged.
