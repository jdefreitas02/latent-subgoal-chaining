"""ReBRAC (TD3+BC, value-based offline RL) on the latent chunked-action datasets.

Same setup/contract as train_iql_chunk.py: only the dynamically-consistent datasets
(original / rollout) are valid; uniform latent goal-reaching reward; standalone chunk25
eval with the deterministic actor. ReBRAC BC alphas are UNTUNED for this task, so treat
this as a cross-check of the FQL (ACFQL) original-vs-rollout result, not a headline.
"""
import argparse
import os
import sys
...
from agents.rebrac_chunk import ReBRACAgent, get_config as get_rebrac_config
...
def main():
    p = argparse.ArgumentParser(description="ReBRAC on latent chunked-action datasets")
    ...
    p.add_argument("--alpha_actor", type=float, default=1.0, help="ReBRAC actor BC coeff (untuned).")
    p.add_argument("--alpha_critic", type=float, default=0.0)
    ...
    z_goal_task = None
    if args.gc:
        (train_dataset_dict, jepa_model, real_envs, z_goals_all) = make_wm_env_and_dataset_multitask(
            wm_ckpt_path=args.wm_ckpt, latent_cache_path=args.wm_cache, hdf5_dataset_path=args.wm_hdf5,
            task_ids=tuple(args.task_ids), done_threshold=args.done_threshold, wm_device=args.wm_device,
        ...
        D = np.asarray(train_dataset_dict["observations"]).shape[-1]; obs_dim = 2 * D
    else:
        (_, _, train_dataset_dict, _, jepa_model, real_env, z_goal_task) = make_wm_env_and_dataset(
            wm_ckpt_path=args.wm_ckpt, latent_cache_path=args.wm_cache, hdf5_dataset_path=args.wm_hdf5,
            task_id=args.task_id, done_threshold=args.done_threshold, wm_device=args.wm_device,
            img_size=224, env_family=args.env_family)
        real_env_eval = real_env; obs_augment = None
        z_goal_task = np.asarray(z_goal_task, dtype=np.float32)
        D = np.asarray(train_dataset_dict["observations"]).shape[-1]; obs_dim = D

    np_rng = np.random.default_rng(args.seed)

    def to_batch(obs, act, next_obs):
        if args.gc:
            g = obs[:, D:]; nz = next_obs[:, :D]
        else:
            g = z_goal_task[None, :]; nz = next_obs
        ...
        return dict(observations=obs.astype(np.float32), actions=act.astype(np.float32),
                    next_actions=act.astype(np.float32),  # placeholder (alpha_critic=0 -> unused)
                    rewards=r, masks=(1.0 - r).astype(np.float32),
                    next_observations=next_obs.astype(np.float32))

    if args.gc:
        # GC: HER sampler over play (original) or the goal-less generated rollout buffer.
        ...
        buffer_dict = dict(observations=obs_all, next_observations=next_obs_all, actions=actions_all,
                           terminals=terms_all, masks=masks_all, ep_id=ep_ids, t_within=t_within)
        sampler = GCHERSampler(task_goals=z_goals_all, done_threshold=args.done_threshold,
                               ...
                               p_randomgoal=args.p_randomgoal, p_taskgoal=args.p_taskgoal, p_geom=args.p_geom)
        n_train = N_data
        def sample_batch():
            b = sampler.sample(buffer_dict=buffer_dict, current_size=N_data, ep_starts=ep_starts,
                               ep_lens=ep_lens, batch_size=args.batch_size, np_rng=np_rng)
            return dict(observations=np.asarray(b["observations"], np.float32),
                        actions=np.asarray(b["actions"])[:, 0, :].astype(np.float32),
                        next_actions=np.asarray(b["next_actions"])[:, 0, :].astype(np.float32),
        ...
        def sample_batch():
            ...
            return to_batch(obs_all[i], act_all[i], next_obs_all[i])

    print(f"=== Creating ReBRAC agent (obs_dim={obs_dim}, gc={args.gc}, {args.condition})...", flush=True)
    ex_obs = np.zeros((1, obs_dim), dtype=np.float32); ex_act = np.zeros((1, 25), dtype=np.float32)
    config = get_rebrac_config()
    config["encoder"] = "jepa_head"; config["lr"] = args.lr
    config["alpha_actor"] = args.alpha_actor; config["alpha_critic"] = args.alpha_critic
    actor_freq = int(config["actor_freq"])
    agent = ReBRACAgent.create(seed=args.seed, ex_observations=ex_obs, ex_actions=ex_act,
                               config=config.to_dict() if hasattr(config, "to_dict") else dict(config))
    print(f"  Agent created. Training data: {n_train} transitions. "
          f"alpha_actor={args.alpha_actor} (UNTUNED).", flush=True)

    logger_train = CsvLogger(os.path.join(args.save_dir, "train.csv"))
    logger_eval = CsvLogger(os.path.join(args.save_dir, "eval.csv"))
    print(f"\n=== ReBRAC training: {args.steps} steps (gc={args.gc}, {args.condition}, seed={args.seed}) ===",
          flush=True)
    ...
    def _do_eval(step):
        if args.gc:
            m = evaluate_real_ogbench(agent=agent, real_env=real_env_eval, jepa_model=jepa_model,
                                      ...
                                      num_episodes_per_task=args.eval_episodes, action_dispatch="chunk25",
                                      pass_task_id_on_reset=False, obs_augment=obs_augment)
        ...
        logger_eval.log(m, step=step); return float(sr)

    for step in range(1, args.steps + 1):
        agent, info = agent.update(sample_batch(), full_update=(step % actor_freq == 0))
        if step % args.log_interval == 0:
            elapsed = time.time() - t_start
            log_data = {k: float(v) for k, v in info.items()}; log_data["elapsed_s"] = elapsed
            logger_train.log(log_data, step=step)
            print(f"  step {step:7d}/{args.steps}  critic={info.get('critic/critic_loss', float('nan')):.3f}  "
                  f"q={info.get('critic/q_mean', float('nan')):.3f}  "
                  f"bc={info.get('actor/bc_loss', float('nan')):.3f}  elapsed={elapsed:.0f}s", flush=True)
    ...
    print(f"\n=== Done. best_SR={best_sr*100:.1f}%. Final params_{args.steps}.pkl in {args.save_dir}/", flush=True)
